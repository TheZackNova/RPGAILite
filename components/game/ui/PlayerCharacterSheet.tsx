                    ...
                    {learnedSkills.map(skillName => (
                        <li key={skillName}>
                            <button onClick={() => onEntityClick(skillName)} className="text-left w-full p-2 bg-slate-200/50 dark:bg-slate-800/50 rounded-md hover:bg-slate-300/50 dark:hover:bg-slate-700/50 transition-colors">
                                ...
                                {knownEntities[skillName]?.description && <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 pl-6">{knownEntities[skillName].description}</p>}
                            </button>
                        </li>
                    ))}
                </ul>
...
const MemoizedStatusesSection = memo<{
    ...
    return (
        <div>
            <h4 className="font-semibold text-lg text-slate-800 dark:text-white mb-2 border-b border-slate-300 dark:border-slate-600 pb-1 flex items-center gap-2">
                <GameIcons.HeartIcon className="w-5 h-5" /> Trạng thái hiện tại
            </h4>
            {statuses.length > 0 ? (
                <div className="flex flex-wrap items-center gap-2 pl-2">
                    {statuses.map(status => (
                        ...
                        >
                            ...
                            <span className={`${getStatusTextColor(status)} ${getStatusFontWeight(status)} text-sm`}>{status.name}</span>
                        </button>
                    ))}
                </div>
...
export const MemoizedPlayerCharacterSheet: React.FC<{
    ...
    return (
        <div className="p-4 h-full flex flex-col space-y-4 text-slate-700 dark:text-gray-300">
            ...
            <MemoizedSkillsSection learnedSkills={learnedSkills} knownEntities={knownEntities} onEntityClick={onEntityClick} />
            <MemoizedStatusesSection statuses={statuses} onStatusClick={onStatusClick} />
        </div>
    );
});
