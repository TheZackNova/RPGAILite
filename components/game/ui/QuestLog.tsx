            ...
            <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                    ...
                    {activeQuests.length > 0 ? (
                        <ul className="space-y-2">
                            {activeQuests.map((quest, index) => (
                                <li key={`active-${quest.title}-${index}`} onClick={() => onQuestClick(quest)} className="text-sm p-2 bg-yellow-400/10 dark:bg-yellow-500/10 border-l-4 border-yellow-600 dark:border-yellow-400 rounded-r-md hover:bg-yellow-400/20 dark:hover:bg-yellow-500/20 transition-colors cursor-pointer">
                ...
                {finishedQuests.length > 0 && (
                    <div className="pt-2">
                        ...
                        <ul className="space-y-2">
                            {finishedQuests.sort((a,b) => a.title.localeCompare(b.title)).map((quest, index) => (
                                <li key={`finished-${quest.title}-${index}`} onClick={() => onQuestClick(quest)} className="text-sm p-2 bg-slate-200/50 dark:bg-slate-700/50 border-l-4 border-slate-400 dark:border-slate-500 rounded-r-md hover:bg-slate-300/50 dark:hover:bg-slate-600/50 transition-colors cursor-pointer opacity-70">
