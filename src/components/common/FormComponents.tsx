...
export const FormLabel: React.FC<FormLabelProps> = ({ htmlFor, children }) => (
    <label htmlFor={htmlFor} className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1">
        {children}
    </label>
);
...
export const CustomSelect: React.FC<CustomSelectProps> = ({ value, onChange, name, children }) => (
    <select 
        name={name} 
        value={value} 
        onChange={onChange} 
